...
template <class T> long extract_data_recived(T task, std::vector<const char**> message_received)
{
  ...
  for (int i = 0; i < message_received.size(); i++) {
    ...
    // XBT_INFO("task name %s massage %s pos %d ", task.name.c_str(), msg.substr(5, sizeof(msg)).c_str(), pos);
...
{
  // note pass by reference to save the status because without it after the call of the function the state is not saved
  // host_name->execute(task.FLOPS);
  ...
  while (cycle) {
    ...
    if (task.in_ctrl_mailboxes.size() >= finished_mailboxes.size() && !finished_mailboxes.empty()) {
      ...
      if (task.streaming) {
        ...
        task.data_size        = extract_data_recived(task, data_message_received);
        execute_send_ctrl_data(task);
        check_do_cycle(task, derive_backward_mailboxes(finished_mailboxes), App_msg_waited_from_Data, cycle);
      }
    }
    ...
    if (task.in_ctrl_mailboxes.size() == nb_expired_mailboxes.size()) {
      task.set_status("EXPIRED");
      sending_ctrl_messages(task, task.out_ctrl_mailboxes);
      break;
    }
